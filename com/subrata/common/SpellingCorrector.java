...
public class SpellingCorrector {
	...
	public static String correct(String word) {
		...
		// Here we can either iterate through list of edits and find the 1st
		// word that is in dictionary and return.
		// Or we can go to below logic to return the word with most weight (that
		// we would have already stored in dictionary map)
		...
		// one we have found possible matches - we want to return most popular
		// (most weight) word
		if (candidates.size() > 0)
			return candidates.get(Collections.max(candidates.keySet()));

		// If none matched.
		// We will pick every word from edits and again do edits (using same
		// previous logic) on that to see if anything matches
		// We don't do recursion here because we don't the loop to continue
		// infinitely if none matches
		// If even after doing edits of edits, we don't find the correct word -
		// then return.
	...
	// It is the edit distance between two words: the number of edits it would
	...
	public static List<String> edits(String word) {
		...
		for (int i = 0; i < word.length() - 1; i++) { // < w.len -1 :: -1
														// because we swapped
														// last 2 elements in
														// go.
		...
		for (int i = 0; i < word.length(); i++) {
			for (char c = 'a'; c <= 'z'; c++) {
				w = word.substring(0, i) + c + word.substring(i + 1); // replacing
																		// ith
																		// char
																		// with
																		// all
		...
		for (int i = 0; i <= word.length(); i++) { // <= because we want to
													// insert the new char at
													// the end as well
			for (char c = 'a'; c <= 'z'; c++) {
				w = word.substring(0, i) + c + word.substring(i); // inserting
																	// new char
																	// at ith
																	// position
				list.add(w);
