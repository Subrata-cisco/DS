...
public class SuffixArray {
	...
	public int lcp(int i) {
		if (i < 1 || i >= suffixes.length)
			throw new IndexOutOfBoundsException();
		int lcp = lcp(suffixes[i], suffixes[i - 1]);
		//System.out.println("#@#@##@#@##@#@#@#@#@#@@# LCP of suffix suffixes[i] ::" + suffixes[i] + " is lcp ::" + lcp);
		return lcp;
	}
	...
	private static int lcp(Suffix s, Suffix t) {
		...
		for (int i = 0; i < n; i++) {
			if (s.charAt(i) != t.charAt(i)) {
				//System.out.println("****  1 LCP s :" + s + " t:" + t + " and lcp is ::" + n);
				return i;
			}

		}
		//System.out.println("****  2 LCP s :" + s + " t:" + t + " and lcp is ::" + n);
		return n;
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (i == 0) {
				...
				System.out.println("word :"+ith+" lcp :"+lcp+" rank :"+rank);
